...
class RegexValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if(empty($this->_methodArray)){
            throw new MethodNotAllowedHttpException('Configuration error:no validating method are found!'.get_class($this),'404');
        }
        foreach($this->_methodArray  as $method){
            if(!$this->hasMethod($method)){
                throw new MethodNotAllowedHttpException('Calling unknown method:'.get_class($this),'404');
            }
        }
    }
